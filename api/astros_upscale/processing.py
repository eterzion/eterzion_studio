...
# degrade gracefully (see profile resolution in astros_upscale_api), not assume every GPU
# reports VRAM.
...
class FaceEnhancer:
    ...
    def restore(self, img_bgr: np.ndarray, strength: float = 1.0) -> tuple[np.ndarray, int]:
        ...
        signature so callers (Upscaler/AstrosUpscaler) don't need to change their

...
# reach astros_licensing_service right now (see check_gate() below); while
...
# astros_licensing_service can't be reached at all — FR-061: a communication
...
# Statuses a license row (astros_licensing_service's own licensing module)
...
# backend that resolves a model (astros_upscale.processing.resolve_model) MUST
...
# Every entry here MUST have a live counterpart in astros_upscale.processing.MODELS;
...
# Every key here MUST match a key in astros_upscale.processing.MODELS — enforced
...
# tracked in astros_upscale.processing.REMOVED_MODEL_IDENTIFIERS, not here.
...
def verify_registry_completeness() -> list[str]:
    ...
    problems (empty = clean) between this registry and astros_upscale's live
...
# the underlying registries this reads from — `astros_upscale.processing.MODELS`
...
# (docs/models/BENCHMARK_RESULTS.md), finalized in T024 — astros_upscale.processing.MODELS

...
the eterzion_upscale_api side for how it's consumed.

Usage: python tools/build_package.py
Run from api/eterzion_licensing_service, with eterzion_upscale_api as a
...
# `module.Upscaler`.
_SOURCE_PATH = Path(__file__).resolve().parent.parent.parent / 'eterzion_upscale_api' / 'app' / 'processing.py'

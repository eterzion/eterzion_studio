...
def ensure_speech_weights(model_dir: str) -> dict[str, str]:
    ...
    return {
        nome: download_with_fallback(
            _com_espelho(nome, _LAVASR_ORIGIN + nome),
...
class FaceEnhancer:
    ...
    def __init__(self, model_dir: str = 'models'):
        weights_path = download_with_fallback(
            _com_espelho('face_detection_yunet_2023mar.onnx', _YUNET_URL), model_dir=model_dir,
...
# Espelho dos modelos: tentado ANTES da fonte original, que vira reserva.
# Motivo concreto: o Hugging Face respondeu 429 (Too Many Requests) a uma
# instalacao real, e um app que depende do limite de taxa de terceiros falha
# por motivo que ninguem aqui controla. Os bytes sao os mesmos -- o SHA-256
# fixado em cada entrada recusa qualquer arquivo diferente, venha de onde vier.
#
# O espelho e' PLUGAVEL, nao um endereco fixo. O app instala um fornecedor que
# monta a URL assinada do CDN privado do Studio (app/cdn.py: cdn.eterzion.com/
# studio/models/, so' com licenca ativa). Sem fornecedor -- a biblioteca usada
# sozinha, pela linha de comando -- o download vai direto a' fonte original.
# Ate' 2026-09-12 o espelho era a release `models-v1` de um repositorio publico
# do GitHub, desativado quando a distribuicao passou para o R2.
_mirror_url_factory: Callable[[str], str | None] | None = None


def set_mirror_url_factory(factory: Callable[[str], str | None] | None) -> None:
    """Define quem monta a URL de espelho de um arquivo (ou None para nenhum)."""
    global _mirror_url_factory
    _mirror_url_factory = factory


def _mirror_url_for(filename: str) -> str | None:
    if _mirror_url_factory is None:
        return None
    try:
        return _mirror_url_factory(filename)
    except Exception as error:  # noqa: BLE001 - espelho indisponivel nunca impede a origem
        logger.warning('espelho indisponivel para %s: %s', filename, error)
        return None


def _com_espelho(filename: str, origem: str) -> list[str]:
    """[espelho, origem] quando ha' espelho; so' [origem] quando nao."""
    espelho = _mirror_url_for(filename)
    return [espelho, origem] if espelho else [origem]


def _load_mirror_map(models_json: str = 'models.json') -> dict:
    """Load a local ``models.json`` mirror override, if present.
...
def _urls_with_mirror(name: str, entry: dict, models_json: str) -> list[list[str]]:
    ...
    senao, do fornecedor instalado (`set_mirror_url_factory`); sem nenhum dos
    dois, a lista tem so' a origem.
    """
    ...
    for url in entry['urls']:
        ...
        candidates.append([mirror_url, url] if mirror_url else _com_espelho(filename, url))
    return candidates
...
def resolve_model(
    ...
    When a local ``models.json`` mirror override (or the installed mirror factory) is

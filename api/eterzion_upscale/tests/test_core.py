...
def test_eterzion_upscaler(tmp_path):
    path = _save_toy_checkpoint(tmp_path, scale=2)

    upscaler = EterzionUpscaler(model_path=path, tile=0, pre_pad=2, half=False, device='cpu')
    ...
    upscaler_tiled = EterzionUpscaler(model_path=path, tile=8, tile_pad=4, half=False, device='cpu')
...
def test_image_io_roundtrip(tmp_path):
    from eterzion_upscale import imread, imwrite, img2tensor, tensor2img

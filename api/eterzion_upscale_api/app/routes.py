...
def _detect_image_content_type(input_path: str) -> str:
    from eterzion_upscale.processing import classify_image
...
def _detect_audio_content_type(input_path: str) -> str:
    """T056 — wires eterzion_upscale.processing's real VAD+harmonic/percussive
    ...
    from eterzion_upscale.processing import classify_audio
...
def _detect_video_content_type(input_path: str) -> str:
    ...
    from eterzion_upscale.media import ffmpeg_path, probe_streams
    from eterzion_upscale.processing import classify_image
...
def _detect_secondary_elements(media_request: MediaRequest, input_path: str) -> dict | None:
    ...
    if media_request.media_type != 'video' or media_request.operation != 'enhance':
        return None
    from eterzion_upscale.media import ProbeError, detect_secondary_elements
...
# eterzion_upscale.processing.MODELS" stop matching "one component per content
# type" (FR-095).
...
# Ja' funcionam sem download: a musica e' melhorada pela masterizacao DSP
# (app.audio_engine), que vem no app. Continua em _NOT_AVAILABLE_IN_APP porque
# nao ha' o que instalar, atualizar ou remover.
_BUILT_IN = frozenset({'music'})
...
# ------------------------------- /license ------------------------------- #
#
# T036 — the local API's facade over eterzion_licensing_service (Constitution

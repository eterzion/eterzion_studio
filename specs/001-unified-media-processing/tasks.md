...
- [ ] T072 Verify the FFmpeg binary the app actually embeds/ships reports no `--enable-gpl`/`--enable-nonfree` in `ffmpeg -version`; replace with an LGPL build if it does; record the verification (date, command output) in `docs/models/MODEL_LICENSES.md` §5

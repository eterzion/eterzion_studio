...
| Fase 3 — Contas, licenciamento, cobrança | ✅ Implementada e testada (webhooks, ativação, autorização) — **não conectada a uma conta real de pagamento** | `interface/astros_licensing_service/` (serviço novo e separado) |
...
- **Hospedagem** — nada disso está publicado em lugar nenhum. `interface/astros_licensing_service` roda local, com SQLite, pronto para virar um serviço de verdade quando houver onde hospedar.

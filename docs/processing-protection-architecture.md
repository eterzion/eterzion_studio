...
|---|---|---|
| Fase 1 — Isolamento de processo | ✅ Implementada e testada | `api/eterzion_upscale_api/app/jobs.py` (as seções de worker supervisor e isolated worker) e `app/security.py` (secure tempdir) |
| Fase 2 — Identidade por instalação | ✅ Implementada e testada | `api/eterzion_upscale_api/app/security.py` (DPAPI + install identity) e a rota de identidade em `app/routes.py` |
| Fase 3 — Contas, licenciamento, cobrança | ✅ Implementada e testada (webhooks, ativação, autorização) — **não conectada a uma conta real de pagamento** | `api/eterzion_licensing_service/` (serviço novo e separado) |
| Fase 4 — Pacote protegido da lógica de orquestração | ✅ Implementada e testada — empacota código-fonte cifrado+assinado, não binário nativo (ver ressalva abaixo) | `api/eterzion_licensing_service/app/packages.py` (a seção de encriptação de pacote) e `app/routes.py` (serviço) + `api/eterzion_upscale_api/app/security.py` (a seção de protected loader, API) |
| Fase 5 — Runtime/anti-adulteração | ✅ Implementada e testada | `api/eterzion_upscale_api/app/security.py` (a seção de self-integrity check e a de protected loader) e `api/eterzion_licensing_service/app/licensing.py` (a seção de authorizations) |
...
- **Hospedagem** — nada disso está publicado em lugar nenhum. `api/eterzion_licensing_service` roda local, com SQLite, pronto para virar um serviço de verdade quando houver onde hospedar.
...
Antes de desenhar qualquer proteção, verifiquei o que `api/eterzion_upscale/processing.py` realmente executa hoje. **Os 19 modelos registrados em `MODELS` são todos pesos públicos de terceiros**, baixados diretamente de URLs públicas do GitHub e do HuggingFace:
...
2. **Só a lógica de orquestração precisa de proteção** — isto é, o código que decide *como* usar os modelos: estratégia de tiling e overlap (`tile_process()` em `api/eterzion_upscale/processing.py`), o limiar de tile e callback de progresso (`Upscaler` em `api/eterzion_upscale_api/app/processing.py`), e a aplicação de ajustes (denoise/sharpen/face recovery).
...
Ponto importante de escopo: **o `eterzion_upscale_api` atual (FastAPI local) não deve virar o serviço remoto**. Ele continua sendo a camada de orquestração local (fila, progresso, export) e passa a delegar só a execução do modelo ao processo isolado. O serviço remoto (Fase 3, §3) é um serviço novo e separado, com seu próprio banco de dados de licenças/instalações/transações — misturar com o `eterzion_upscale_api` quebraria a premissa de que segredos reais (chave de assinatura, dados de licença) nunca tocam a máquina do usuário.
...
**O que entra:** um serviço backend novo (fora do `eterzion_upscale_api`) com três responsabilidades:
...
|---|---|
| `eterzion_upscale_api` (FastAPI local) | Deixa de rodar a lógica de tiling/ajustes no próprio processo; passa a orquestrar o processo isolado (Fase 1) e a se comunicar com o serviço remoto mínimo (Fase 3 simplificada) para obter a autorização/chave de sessão a cada job. |
| `api/eterzion_upscale/processing.py` | `tile_process()` e a lógica de blending/ajustes migram para o módulo protegido que roda dentro do executor isolado. O download e carregamento do peso do modelo em si **não muda** — continua público, sem DRM, exatamente como hoje. |

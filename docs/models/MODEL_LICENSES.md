...
| 2026-09-12 | **Pesos da voz (LavaSR/`audiosronnx`) vão para o disco do usuário pelo app e são redistribuídos pelo espelho público** | O repositório de pesos `TigreGotico/audiosronnx-lavasr` no Hugging Face não tem etiqueta de licença; a base é a declaração do próprio pacote `audiosronnx` 0.9.0a2: "ONNX artifacts: TigreGotico/audiosronnx-lavasr (Apache-2.0)" (docstring de `engines/lavasr.py`) e a coluna de licença da tabela de motores (`lavasr` → Apache-2.0). Revisão fixada `b3df8a26`, SHA-256 fixados no código. Se a etiqueta aparecer diferente no HF, esta linha precisa ser revista |
| 2026-09-12 | **Espelho público dos modelos** em `eterzion/eterzion_studio_releases`, release `models-v1` | Redistribuição sem modificação, permitida por todas as licenças espelhadas (CC-BY-4.0, BSD-3-Clause, Apache-2.0, MIT) com atribuição — autores e licenças no README daquele repositório. O VAE do Stable Audio Open **não** é espelhado: é gated, e espelhá-lo contornaria o aceite dos termos da Stability AI |
| 2026-09-12 | **Espelho dos modelos e atualizações migrados para o CDN privado do Studio** (bucket R2 `eterzion-studio`, servido em `cdn.eterzion.com/studio/` só para licença ativa); `eterzion_studio_releases` desativado | Os mesmos arquivos, sem modificação, conferidos pelo SHA-256 fixado no código (`scripts/models_to_r2.py`). A atribuição de autores e licenças passa a ficar na tela Créditos do app, que acompanha cada cópia. O VAE do Stable Audio Open continua **fora** do espelho |
| 2026-09-12 | **Voz embarcada no instalador** (`audiosronnx` + `onnxruntime`), revisando o FR-023 do spec 006 para a voz | O app empacotado não tem pip; o FR-023 foi escrito para o ambiente de desenvolvimento. A música (SonicMaster) continua fora do instalador |
| 2026-09-13 | **Melhorar música sem SonicMaster**: todo job de música passa pelo `MasteringEngine` (análise + restauração + masterização por DSP, filtros LGPL do FFmpeg); o excitador de harmônicos (`aexciter`) só entra quando o analisador mede o corte de agudos de um arquivo comprimido (< 15,5 kHz) | O SonicMaster exige o VAE do Stable Audio Open (gated, teto comercial de US$ 1 mi, não espelhável) e torch CUDA com 6-8 GB de VRAM — nada disso vai no app, e todo job de música falhava. Nenhum filtro de áudio do FFmpeg está na lista GPL (`LICENSE.md`/`configure` do FFmpeg: só filtros de vídeo). SonicMaster e o VAE saem dos Créditos: o app não os usa nem distribui |
| 2026-09-13 | **Apollo (JusperLee, CC-BY-SA-4.0) avaliado para música comprimida e deixado de fora por ora** | Restaura de fato os agudos de um MP3 de 64 kbps (energia acima de 16 kHz de -71 para -11 dB em relação ao original; LSD 2,1 → 0,9), mas em CPU levou **15,6 min para uma música de 4 min** (RTF 3,9, 16 threads) e 4,5 GB de RAM — o backend do app é só CPU. A masterização DSP leva ~16 s na mesma faixa. Reavaliar se houver caminho de GPU no app. Risco de procedência anotado: treinado no MUSDB18-HQ, de uso acadêmico |
